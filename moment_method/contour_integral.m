function result = contour_integral(freq, x_in, y_in, x1, x2, y1, y2, func)

line_length = sqrt((x2 - x1).^2 + (y2 - y1).^2);
wavelen = (3e+8)./(freq./1000);
ang_freq = 2.*pi.*freq;
k_val = 2.*pi./wavelen;
mu_nought = pi.*4e-7;
slope = (y2 - y1)./(x2 - x1);
...
const_term = -1.*ang_freq.*mu_nought.*line_length./4000;

sum = 0;

for i=1:5
 sum = sum + weights(i).*func(k_val.*sqrt((x_in - 100 - 22.*nodes(i)).^2 + (y_in - 100 + 22.*nodes(i)).^2));
endfor
