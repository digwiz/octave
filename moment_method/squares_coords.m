...
function [squares_column, centers_column] = squares_coords(width, thickness, curve_function, first_x, first_y, num_width_splits)

edge_length = width./num_width_splits;
squares_thick = floor(thickness./square_side);
squares = [];
squares_centers = [];
...
for iter=1:num_width_splits
    squares(squares_count+1,1) = square_corners(first_x,first_y-edge_length.*(iter-1), edge_length);
    squares_centers(squares_count+1,1) = square_centroid(first_x,first_y-edge_length.*(iter-1), edge_length);
...
for iter=1:(squares_thick-1)
    new_first_x = first_x.*(-1).*edge_length.*iter;
    new_first_y = curve_function(new_first_x);
    ...
    for it=1:num_squares_to_add
        squares(squares_count+1,1) = square_corners(new_first_x,new_first_y-edge_length.*(iter-1),edge_length);
        squares_centers(squares_count+1,1) = square_corners(new_first_x,new_first_y-edge_length.*(iter-1),edge_length);
        squares_count = squares_count + 1;
